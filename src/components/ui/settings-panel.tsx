...
interface SettingsPanelProps {
  ...
  onCameraChange: (c: Partial<{ distance: number; followBone: string; followSmoothing: number; offsetY: number }>) => void
  ...
  onSunChange: (s: Partial<{ direction: { x: number; y: number; z: number }; strength: number; color: { r: number; g: number; b: number } }>) => void
...
function ColorPicker({ label, color, onChange }: {
  label: string; color: { r: number; g: number; b: number }; onChange: (c: { r: number; g: number; b: number }) => void
}) {
    ...
    <div className="flex items-center gap-2">
      ...
      <input type="color"
             ...
             onChange={e => {
               ...
               onChange({ r: parseInt(h.slice(1, 3), 16) / 255, g: parseInt(h.slice(3, 5), 16) / 255, b: parseInt(h.slice(5, 7), 16) / 255 })
             }}
  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="flex flex-col gap-1">
        ...
        <Slider label="Distance" value={camera.distance} min={5} max={50} onChange={v => onCameraChange({ distance: v })} />
        <Slider label="Offset Y" value={camera.offsetY} min={-5} max={10} onChange={v => onCameraChange({ offsetY: v })} />
        <Slider label="Smoothing" value={camera.followSmoothing} min={0} max={0.5} step={0.01} onChange={v => onCameraChange({ followSmoothing: v })} />
        <div className="flex items-center gap-2">
          ...
          >
            {FOLLOW_BONES.map(b => <option key={b.value} value={b.value} className="bg-zinc-900">{b.label}</option>)}
          </select>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-1">
        ...
        <Check label="Enabled" checked={background !== null} onChange={v => onBackgroundChange(v ? { r: 0, g: 0.69, b: 0.14 } : null)} />
      ...
      <div className="flex flex-col gap-1">
        ...
        <div className="flex flex-col gap-0.5">
          ...
          <div className="grid grid-cols-3 gap-1">
            ...
            <input type="number" value={sun.direction.x} step={0.05} min={-1} max={1}
            ...
            <input type="number" value={sun.direction.y} step={0.05} min={-1} max={1}
                   onChange={e => onSunChange({ direction: { ...sun.direction, y: Number(e.target.value) } })}
                   className="w-full rounded bg-white/5 border border-white/10 text-[10px] text-white px-1 py-0.5 text-center outline-none" />
            <input type="number" value={sun.direction.z} step={0.05} min={-1} max={1}
      ...
      <div className="flex flex-col gap-1">
        ...
        <Slider label="Strength" value={world.strength} min={0} max={2} onChange={v => onWorldChange({ strength: v })} />
      ...
      <div className="flex flex-col gap-1">
        ...
        <Slider label="Min Cutoff" value={smoothing.minCutoff} min={0.1} max={5} onChange={v => onSmoothingChange({ minCutoff: v })} />
        <Slider label="Beta" value={smoothing.beta} min={0.1} max={5} onChange={v => onSmoothingChange({ beta: v })} />
        <Slider label="dCutoff" value={smoothing.dCutoff} min={0.5} max={10} onChange={v => onSmoothingChange({ dCutoff: v })} />
      </div>
    </div>
  )
}