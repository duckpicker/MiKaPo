  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <div className="flex flex-col gap-1">
        ...
        <Slider
          label="Mouth Open"
          value={thresholds.mouthOpen}
          min={0.05}
          max={0.5}
          onChange={(v) => onThresholdChange({ mouthOpen: v })}
        />
        <Slider
          label="Smile"
          value={thresholds.smile}
          min={0.001}
          max={0.05}
          step={0.001}
          onChange={(v) => onThresholdChange({ smile: v })}
        />
        <Slider
          label="Eye Open"
          value={thresholds.eyeOpen}
          min={0.1}
          max={0.6}
          onChange={(v) => onThresholdChange({ eyeOpen: v })}
        />
        <Slider
          label="Eye Closed"
          value={thresholds.eyeClosed}
          min={0.01}
          max={0.3}
          onChange={(v) => onThresholdChange({ eyeClosed: v })}
        />
      </div>
